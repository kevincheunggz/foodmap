 // 美食地图 Web App - 主逻辑
  ...
  function renderList() {
      ...
      filtered.forEach(r => {
          ...
          html += '<div class="restaurant-name">' + r.name + '</div>';
          html += '<div class="restaurant-address">' + r.address + '</div>';
          html += '<div class="restaurant-meta">';
          if (r.cuisineType) {
              html += '<span class="cuisine-tag">' + r.cuisineType + '</span>';
          }
          if (r.averagePrice) {
              html += '<span class="price-tag">¥' + r.averagePrice + '/人</span>';
          }
          if (r.rating) {
              html += '<span class="rating">' + '⭐'.repeat(Math.floor(r.rating)) + '</span>';
          }
          html += '</div></div>';
  ...
  function saveRestaurant(e) {
      ...
      if (id) {
          ...
          if (index !== -1) {
              state.restaurants[index] = Object.assign({}, state.restaurants[index], restaurant);
          }
      } else {
  ...
  function showDetail(id) {
      ...
      let html = '';
      html += '<div class="detail-item"><div class="detail-label">地址</div>';
      html += '<div class="detail-value">' + restaurant.address + '</div></div>';

      if (restaurant.cuisineType) {
          html += '<div class="detail-item"><div class="detail-label">菜系</div>';
          html += '<div class="detail-value">' + restaurant.cuisineType + '</div></div>';
      }

      if (restaurant.averagePrice) {
          html += '<div class="detail-item"><div class="detail-label">人均消费</div>';
          html += '<div class="detail-value">¥' + restaurant.averagePrice + '</div></div>';
      }

      if (restaurant.phone) {
          html += '<div class="detail-item"><div class="detail-label">联系电话</div>';
          html += '<div class="detail-value">' + restaurant.phone + '</div></div>';
      }

      if (restaurant.businessHours) {
          html += '<div class="detail-item"><div class="detail-label">营业时间</div>';
          html += '<div class="detail-value">' + restaurant.businessHours + '</div></div>';
      }

      if (restaurant.rating) {
          html += '<div class="detail-item"><div class="detail-label">评分</div>';
          html += '<div class="detail-value">' + '⭐'.repeat(Math.floor(restaurant.rating)) + ' (' + restaurant.rating +
   '/5)</div></div>';
      }

      if (restaurant.videoUrl) {
          html += '<div class="detail-item"><div class="detail-label">B站视频</div>';
          html += '<div class="detail-value"><a href="' + restaurant.videoUrl + '" target="_blank" style="color:
  #FF6B35;">';
          html += (restaurant.videoTitle || '点击查看视频') + ' ↗</a></div></div>';
      }

      if (restaurant.notes) {
          html += '<div class="detail-item"><div class="detail-label">个人备注</div>';
          html += '<div class="detail-value">' + restaurant.notes + '</div></div>';
      }

      html += '<div class="detail-item"><div class="detail-label">打卡状态</div>';
      html += '<div class="detail-value">' + (restaurant.hasVisited ? '✅ 已打卡' : '⭕ 未打卡') + '</div></div>';
  ...
  function navigateToRestaurant(restaurant) {
      ...
      const url = 'amapuri://route/plan/?dlat=' + restaurant.latitude + '&dlon=' + restaurant.longitude + '&dname=' +
  encodeURIComponent(restaurant.name) + '&dev=0&t=0';
      ...
      setTimeout(() => {
          if (Date.now() - startTime < 2100) {
              const webUrl = 'https://uri.amap.com/navigation?to=' + restaurant.longitude + ',' + restaurant.latitude +
  ',' + encodeURIComponent(restaurant.name) + '&mode=car&coordinate=gaode';
  ...
  async function syncData(showUI) {
      if (showUI) {
      ...
      try {
          ...
          if (showUI) {
              ...
              if (newCount > 0) {
                  resultEl.innerHTML = '<strong>更新成功！</strong><br>新增 ' + newCount + ' 家店铺';
                  resultEl.className = 'sync-result has-update';
              } else {
                  resultEl.innerHTML = '<strong>已是最新</strong><br>没有新店铺';
  ...
  function mergeData(local, remote) {
      ...
      remote.forEach(r => {
          const exists = merged.some(m => m.name === r.name && m.address === r.address);
          if (!exists) {
              merged.push(Object.assign({}, r, { isNew: true }));
          }
      });
